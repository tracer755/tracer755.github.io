  ...
  `;
        ...
        element.documents.forEach(element => {
          ...
          <div style="padding-top:12px; padding-bottom:12px; padding-left:6px;">
              <a class="orangelink" id="${element.title}" href="${element.url}" target="_blank"><h4 class="orangelink" style="color: white;">${element.title}</h4></a>
          </div>`;
            document.getElementById(itemcontainerid).innerHTML += linkitemtemplate;
            let tmp = String(element.url)
            if (tmp.includes("./")) {
              document.getElementById(element.title).target = "_self"
            }
          } else {
